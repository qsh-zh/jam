...
class Coloredtqdm(tqdm):
    ...
    def set_postfix(self, ordered_dict=None, refresh=True, color=True, round=4, **kwargs):
        ...
        if color:
            self.postfix = ", ".join(value_color[key]+key+"="+postfix[key]+COLORS.END_NO_TOKEN for key in postfix.keys())
        else:
