...
def train_bar_init(trainer):
    ...
    trainer.epoch_bar = tqdm(
        total=trainer._cfg.epochs, desc="epochs", dynamic_ncols=True, initial=trainer.epoch_cnt, file=sys.stdout
    )
    trainer.batch_bar = tqdm(
        total=trainer.num_batch, leave=False, desc="train", dynamic_ncols=True, file=sys.stdout
    )


def batch_bar_update(trainer):
    trainer.batch_bar.update()
    trainer.batch_bar.set_postfix(dict(total_it=trainer.iter_cnt, loss=trainer.batch_loss))

def epoch_bar_update(trainer):
    trainer.epoch_bar.update()
    trainer.epoch_bar.set_postfix(dict(total_it=trainer.iter_cnt, loss=trainer.batch_loss))
...
def val_start(trainer):
    ...
    trainer.val_bar = tqdm(total=len(trainer.val_loader), leave=False, desc="val",dynamic_ncols=True, file=sys.stdout)
